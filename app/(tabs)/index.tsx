...
export default function ChatListScreen() {
  ...
  const fetchCharacters = async () => {
    ...
    if (data && data.length > 0) {
      ...
      unlockInitialCharacters(user.id);
    }
  };

  const unlockInitialCharacters = async (userId: string) => {
    const { data: mom } = await supabase.from('characters').select('*').eq('name', 'Mom').single();
    if (mom) {
        await supabase.from('user_characters').insert({ user_id: userId, character_id: mom.id }).ignore();
    }
    const { data: dad } = await supabase.from('characters').select('*').eq('name', 'Dad').single();
    if (dad) {
        await supabase.from('user_characters').insert({ user_id: userId, character_id: dad.id }).ignore();
    }
    ...
    >
      <View className="w-12 h-12 rounded-full bg-gray-300 items-center justify-center overflow-hidden mr-4">
         {item.characters.avatar_url ? (
            <Image source={{ uri: item.characters.avatar_url }} className="w-full h-full" />
         ) : (
